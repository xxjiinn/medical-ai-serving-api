...
### 3.4 인구통계별 데이터 가용성 문제

**발견: 25-40세 여성 데이터 심각한 누락**

| 연령대     | 성별 | 원본 레코드 | 유효 레코드 | 유효율  | 주요 원인                    |
| ---------- | ---- | ---------- | ----------- | ------- | ---------------------------- |
| 25-29세    | 여성 | 18,158     | 0           | 0.0%    | Total Cholesterol 100% 누락  |
| 30-34세    | 여성 | 32,814     | 0           | 0.0%    | Total Cholesterol 100% 누락  |
| 35-39세    | 여성 | 40,427     | 1           | 0.002%  | Total Cholesterol 99.99% 누락 |
| 40-44세    | 여성 | 29,676     | 0           | 0.0%    | Total Cholesterol 100% 누락  |
| **소계**   | 여성 | **121,075**| **1**       | **0.001%** | -                          |

**근본 원인:**
- 원본 CSV에서 25-40세 여성 121,074명의 총콜레스테롤(Total Cholesterol) 및 HDL콜레스테롤 데이터가 누락
- 검증 로직은 올바름: 혈청 지질 데이터가 없으면 `invalid_flag=True` 처리
- 데이터 출처(국민건강보험공단)에서 이 인구통계에 대한 콜레스테롤 정보 미수집 가능성

**영향:**
- `clean_risk_result` 테이블에 25-40세 여성은 사실상 결과 없음 (1명 제외)
- 이 연령대/성별 리스크 프로파일링 불가
- 통계 및 필터링 쿼리에서 여성 비율 왜곡 (실제 45.5% → 유효 데이터 45.2%)

**대응:**
- 지질 검사 필드를 필수에서 선택으로 변경하지 않음 (의료 지침상 필수)
- 대신 UI에서 이 인구통계에 대한 데이터 부족 명시 (투명성)
- 미래 데이터셋에서 문제 개선 필요
